...
*/
const gtlRequest = (page, url, headers, form_data) => page.evaluate(async (headers, url, form_data) => {
    let formBody = [];
...
*/
const createRequestBody = (postData, { service = "AdvancePay", facilityState = "FL", facility = "1010", phoneNumber = "(310) 333-3338", hour = "3", minute = "00", amPm = "AM", callDuration = "1", }) => {
    ...
    const facilityK = keys.find((k) => k.includes("facility") && !k.includes("State"));
...
Apify.main(async () => {
    ...
    const crawler = new Apify.PuppeteerCrawler({
        ...
        handlePageFunction: async ({ page }) => {
            await login(page);
            const { headers, postData } = await getHeaders(page);
            console.log("HEADERS", headers);
            console.log("postData", postData);
            const result = await gtlRequest(page, "https://www.connectnetwork.com/webapp/jsps/cn/ratesandfees/landing.cn", {
                ...headers,
                "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
